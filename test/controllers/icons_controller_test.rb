...
class IconsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create icon" do
    assert_difference('Icon.count') do
      post icons_url, params: { icon: { code: @icon.code, local_name: @icon.local_name, name: @icon.name } }
    end
  ...
  test "should update icon" do
    patch icon_url(@icon), params: { icon: { code: @icon.code, local_name: @icon.local_name, name: @icon.name } }
